...
class AdminWindowsScreen extends StatelessWidget {
  AdminWindowsScreen({super.key});

  Future<void> _deleteUser(
      BuildContext context, UserRemoteModel user, int index) async {
        ...
        return Scaffold(
            ...
            body: Center(
              child: Padding(
                ...
                child: SizedBox(
                  height: mediaQuerySize.height * 1.5,
                      ...
                      CustomScrollView(
                        slivers: [
                          ...
                          SliverList.separated(
                            itemCount: state.isSearchingNow
                                ? state.sortedUserList.length
                                : state.users.length,
                            itemBuilder: (context, index) {
                              return UserWidget(
                                userRemoteModel: state.isSearchingNow
                                    ? state.sortedUserList[index]
                                    : state.users[index],
                                deleteUser: () =>
                                    _deleteUser(context, state.users[index], index),
                                editUser: () => getIt<UserBloc>()
                                    .add(UserEvents.openEditUserForm(index)),
                                copyUserAuthId: () => getIt<UserBloc>()
                                    .add(UserEvents.copyUserAuthId(index)),
                              );
                            },
